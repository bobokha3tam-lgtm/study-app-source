...
app.post("/api/advisor/generate-schedule", requireAuthorizedUser, async (req, res) => {
  const { profile, focusNotes, examBudget, reasoningMode, examErrors, recentReports, planningStrategy, spacedReviewEnabled } = req.body;
  const ai = getGenAI();
...
۶. ادغام قطعی کلاس‌های هفتگی و بودجه‌بندی آزمون:
- کلاس‌های هفتگی دانش‌آموز خط قرمز هستند و در ساعات مربوطه باید قرار گیرند.
